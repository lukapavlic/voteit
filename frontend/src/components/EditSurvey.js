...
const EditSurvey = () => {
    ...
    const postData = async (survey) => {
        ...
        const res = await fetch(
            process.env.REACT_APP_SURVEYS_BACKEND,
            {
                method: 'POST',
                headers: {
    ...
    const addSurvey = (e) => {
        ...
        postData(surveyToAdd)
        navigate('/mysurveys')

    }

    useEffect(() => {
        const n = urlParams.get('newSurveyName')
        if (n !== null) setName(n)
    }, [])
    ...
    return (
        <form>
            ...
            <Grid container spacing='2'>
                ...
                <Grid item xs='3'><Button variant='contained' onClick={addSurvey}>Save</Button></Grid>
                <Grid item xs='9'></Grid>
            </Grid>
